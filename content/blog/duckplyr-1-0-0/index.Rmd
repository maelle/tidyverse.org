...
Like with other dplyr backends like dtplyr and dbplyr, duckplyr allows you to get results faster.
...
The example below also works when the duckplyr package is not attached:
...
In both cases, printing the result only computes and shows the first few rows, as with dbplyr.

```{r}
out
```

For programming, the resulting object is indisguishable from a regular tibble, except for the additional class.
...
```{r}
babynames |>
  filter(n == max(n), .by = c(year, sex))
```

Over time, we expect fewer fallbacks to dplyr to be needed.
